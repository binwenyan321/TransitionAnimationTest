...
enum TransitionThreeType {
    case push
    case pop
}

class TransitionThree: NSObject {
    
    // MARK: - Public Property
    var type: TransitionThreeType = .push
    
    // MARK: - Override
    deinit {
        NSLog("TransitionThree dealloc")
    }
}

// MARK: Private Method
extension TransitionThree {
    /// push动效
    fileprivate func pushAnimation(transitionContext: UIViewControllerContextTransitioning) {
        ...
        guard let toVC = transitionContext.viewController(forKey: UITransitionContextViewControllerKey.to) as? ViewControllerThree
        else {
...
//        let fromView = transitionContext.view(forKey: UITransitionContextViewKey.from)
    ...
    fileprivate func popAnimation(transitionContext: UIViewControllerContextTransitioning) {
        // 通过viewControllerForKey取出转场前后的两个控制器，这里toVC就是转场后的VC、fromVC就是转场前的VC
        guard let fromVC = transitionContext.viewController(forKey: UITransitionContextViewControllerKey.from) as? ViewControllerThree
        else {
            handleUnexpectationState(transitionContext: transitionContext)
            return
        }
...
extension TransitionThree: UIViewControllerAnimatedTransitioning {
    /// 返回动画时长
