...
enum TransitionTwoType {
    case present
    case dismiss
}

class TransitionTwo: NSObject {
    
    // MARK: - Public Property
    var type: TransitionTwoType = .present
    
    // MARK: - Override
    deinit {
        NSLog("TransitionTwo dealloc")
    }
}

// MARK: Private Method
extension TransitionTwo {
    /// present动效
    fileprivate func presentAnimation(transitionContext: UIViewControllerContextTransitioning) {
        guard let toVC = transitionContext.viewController(forKey: UITransitionContextViewControllerKey.to) as? ViewControllerTwo,
    ...
    fileprivate func dismissAnimation(transitionContext: UIViewControllerContextTransitioning) {
        ...
        guard let fromVC = transitionContext.viewController(forKey: UITransitionContextViewControllerKey.from) as? ViewControllerTwo
        else {
            handleUnexpectationState(transitionContext: transitionContext)
            return
        }
...
extension TransitionTwo: UIViewControllerAnimatedTransitioning {
    /// 返回动画时长
